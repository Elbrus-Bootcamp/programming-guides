# Деплой на Selectel с использованием Portainer (для управления контейнерами), React SSR на Express
...
6. Прописать подключение к базе данных в режиме `production` и `development`. Лучше
...
3. В корне сервера выполнить `docker build . -t [имя образа]:[версия]` и затем проверить
...
10. Отправить образ в Docker Hub:
...
> появляться ошибка. Для отката к последней на данной момент стабильной версии Docker
...
> версий Docker Скопируй требуюмую версию. На момент написания комментария рабочая версия
...
15. На локальном компьютере заполняем переменные окружения для `production` базы данных:
